...
def scaled_input(minimum_weight, maximum_weights, batch_size, num_batch):
    ...
    res = torch.cat([torch.add(minimum_weight, step * i) for i in range(num_points)], dim=0)  # (num_points, ffn_size)
    return res, step[0]
...
def get_context_attr(idx, prompt_without_context, prompt_with_context, answer_obj, args, model, tokenizer, device):
    ...
    for tgt_layer in range(model.model.config.num_hidden_layers):
        ...
        def wo_forward_hook_fn(module, inp, outp):
            wo_ffn_weights_dict['input'] = inp[0]  # inp type is Tuple
        ...
        wo_ffn_weights = wo_ffn_weights_dict['input']
        ...
        w_ffn_weights = w_ffn_weights_dict['input']
        ...
        for batch_idx in range(args.num_batch):
            ...
            for i in range(0, args.batch_size, args.batch_size_per_inference):
                ...
                change_hook.remove()  # check_ffn_weights_dict['output'][:,-1,:]
